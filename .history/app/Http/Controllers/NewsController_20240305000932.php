...
class NewsController extends Controller
{
    ...
    public function store(Request $request) {
        $data= $request->validate([
            ...
            'image' => 'required|image|file_extension:jpeg,jpg,png,jfif,svg|max:2048'
            ],
            #errors
            [
                'image' . 'image' => "The image field must be an image.",
                'image' . 'file_extension' => "The image field must be an image with extention jpeg, jpg, png, jfif, or svg.",
        ]);
    ...
    public function update(Request $request, $id)
    {
        ...
        if (auth()->user() && ((auth()->user()->role == 'admin') || (auth()->user()->role == 'hero' && auth()->user()->id == $news->user_id))) {
            $data = $request->validate([
                'title' => 'required|string',
                'content' => 'required|string',
                'image' => 'image|file_extension:jpeg,jpg,png,jfif,svg|max:2048',
            ], [
                'image.image' => "The image field must be an image.",
                'image.file_extension' => "The image field must be an image with extension jpeg, jpg, png, jfif, or svg.",
            ]);
