<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>My Travel</title>
<link rel="stylesheet" href={{ asset('css/style.css') }}>
</head>
<body>
<header>
    
        <nav>
            <ul>
                {{-- <li class="first">Ho<span class="logo">me</span></li> --}}
                <li class="first"><img src="{{  asset('img/nav.png') }}" alt="Heros" /></li>
                <li class="second"><a href="/">Home</a></li>
                <li class="second"><a href="#">Information</a></li>
                <li class="second"><a href="about">About</a></li>
                <li class="second"><a href="#">Culture</a></li>
                <li class="second"><a href="#">Content</a></li>
                <li class="second"><a href="login">Log out</a></li>
            </ul>
        </nav>


</header>
    <div class="content1">

        <h3>LET'S</h3>
        <br>
        <h1>planning your<br>own travel </h1>
        <br>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit.
        <br>Voluptate debitis dignissimos veritatis hic assumenda estu</p>
        <br>
        <button class="btn">book a trip</button>
    </div>
    <div class="tours">
    <div >
        <img class="nap1" src="img/nap1.jpg" alt="ski tours">
        <p>ski tours</p>
    </div>
    <div>
        <img src="img/nap2.jpg" alt="ski tours">
        <p>ski tours</p>
    </div>
    <div>
        <img src="img/nap3.jpg" alt="ski tours">
        <p>ski tours</p>
    </div>
    <div>
        <img src="img/nap4.jpg" alt="ski tours">
        <p>ski tours</p>
    </div>
    <div>
        <img src="../img/nap5.jpg" alt="ski tours">
        <p>ski tours</p>
    </div>
    </div>
    <div class="content2">
        <img src="img/h1.png" alt="mountain">
    <div>
        <h3>EAT NUSA TENGGA</h3>
        <br>
        <h2>have uou enjoyed your holiday</h2>
        <br>
        <p>Lorem ipsum, dolor sit amet consectetur adipisicing elit. 1110 inventore accusamus, explicabo impedit laboriosam vero
        <br> autem recusandae illum quo. Beatae ipsa inventore aliquid ipsum officia.</p>
        <br>
        <button class="btn">Read more</button>
    </div>
    </div>
    <div class="content3">
    <div class="t1 background-img">
        <p>East Java</p>
        <p class="p">mont boro</p>
    </div>
    <div class="t2 background-img">
        <p>East Java</p>
        <p class="p">mont boro</p>
    </div>
    <div class="t3 background-img">
        <p>East Java</p>
        <p class="p">mont boro</p>
    </div>
    </div>

    <center><button class="btn">See Tours</button></center>

<footer>
    <p>copyright	&copy; 2024</p>
</footer>
</body>
</html>
