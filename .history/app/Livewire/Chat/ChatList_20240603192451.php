<?php

namespace App\Livewire\Chat;

use App\Models\Conversation;
use App\Models\User;
use Livewire\Component;

class ChatList extends Component
{
    public $selectedConversation;

    public $query;

    protected $listeners = ['refresh' => '$refresh'];

    public function deleteByUser($id)
    {

        $userId = auth()->user()->id;
        $conversation = Conversation::find(decrypt($id));

        $conversation->messages()->each(function ($message) use ($userId) {

            if ($message->sender_id === $userId) {

                $message->update(['sender_deleted_at' => now()]);
            } elseif ($message->receiver_id === $userId) {

                $message->update(['receiver_deleted_at' => now()]);
            }
        });

        $receiverAlsoDeleted = $conversation->messages()
            ->where(function ($query) use ($userId) {

                $query->where('sender_id', $userId)
                    ->orWhere('receiver_id', $userId);
            })->where(function ($query) use ($userId) {

                $query->whereNull('sender_deleted_at')
                ->orWhereNull('receiver_deleted_at');
            })->doesntExist();

        if ($receiverAlsoDeleted) {

            $conversation->forceDelete();
            # code...
        }

        return redirect(route('chat.index'));
    }

    public function render()
    {
        $user = auth()->user();
        return view('livewire.chat.chat-list', [
            'conversations' => $user->conversations()->latest('updated_at')->get(),
            'query' => $this->query,
            'selectedConversation' => $this->selectedConversation,
        ]);
    }
    // public function render()
    // {
    //     // Get the authenticated user
    //     $user = auth()->user();

    //     // Ensure the user is authenticated
    //     if (!$user) {
    //         // Handle unauthenticated user case, e.g., redirect or show an error
    //         return redirect()->route('login');
    //     }

    //     // Retrieve the user's conversations, ordered by the latest update
    //     $conversations = User::conversations()->latest('updated_at')->get();

    //     // Render the Livewire view with the necessary data
    //     return view('livewire.chat.chat-list', [
    //         'conversations' => $conversations,
    //         'query' => $this->query,
    //         'selectedConversation' => $this->selectedConversation,
    //             
    //     ]);
    // }
}
