...
class NewsController extends Controller{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            $news = DB::table('news')
                ->join('users', 'news.user_id', '=', 'users.id')
                ->leftJoin('news_tags', 'news.id', '=', 'news_tags.news_id') // Left join with news_tags table
                ->leftJoin('tags', function ($join) {
                    $join->on('news_tags.tag_id', '=', 'tags.id')
                    ->whereColumn('news_tags.news_id', '=', 'news.id');
                }) // Left join with tags table with conditions
                ->select('news.*', 'users.firstName', 'users.lastName', 'users.role', 'tags.*') // Select all columns from tags table
                ->orderBy('news.created_at', 'desc')
                ->get();
            dd($news)
