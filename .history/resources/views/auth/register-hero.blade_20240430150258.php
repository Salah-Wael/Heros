...
@section('sign')
    <form style="height:440px;margin-top:-60px" action="/register-hero" method="POST">
        ...
        <div class="form-group">
            <label class="form-label" for="country">Bi Country</label>
            <select id="country" name="country">
                @foreach ($countries as $country)
                    <option class="form-control mb-0" value="{{ $country->id }}">{{ $country->country }}</option>
                @endforeach
            </select>
            {{-- <input type="date"  value="{{ old('birthDate') }}"  id="birthDate" name="birthDate" required> --}}
        </div>
        <div class="form-group">
