...
class NewsController extends Controller{
    ...
    public function store(Request $request) {
        $data = validator::make($request->all(),
            ...
            [
                ...
                'image' . 'mimes' => "The file field must be an image with extention (jpeg, jpg, png, jfif, or svg).",
            ]
        );
    ...
    public function index(Request $request){
        ...
        if ($request->has('search')) {
            $news = $this->searchAboutNews($request);
        }
        else{
            $news = News::with('tags')
            ->join('users', 'news.user_id', '=', 'users.id')
        ...
        if(auth()->user() && ((auth()->user()->role == 'admin') || (auth()->user()->role == 'hero' && auth()->user()->id == $news->user_id))) {
            ...
            switch ($userRole) {
                case auth()->user()->role == 'admin':
                    ...
                    return response()->json([
                        'status' => 200,
                        'success' => "Chosen news deleted successfully",
                        'role' => 'admin'
                    ]);
                    break;
                ...
                case auth()->user()->role == 'hero':
                    ...
                    return response()->json([
                        'status' => 200,
                        'success' => "Your news deleted successfully",
                        'role' => 'hero'
                    ]);
                    break;
