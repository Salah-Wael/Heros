...
<form action="{{ route('news.update',$news->id) }}" method="post" enctype="multipart/form-data" class="dropzone" id="dropzone">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-9">
            {{-- <input class="form-control" type="file" value="{{ $news->file('image') }}" name="image" id="image" accept=".jpeg, .jpg, .png, .jfif, .svg"  required> --}}
            <label class="custom-file-input-label" id="imageLabel">Choose a file</label>
            <input class="form-control" type="file" name="image" id="image" accept=".jpeg, .jpg, .png, .jfif, .svg" onchange="displayFileName(this)" required>
        </div>
    </div>
