...
class NewsController extends Controller{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            $news = DB::table('news')
                ->join('users', 'news.user_id', '=', 'users.id')
                ->leftJoin('tags', 'news.id', '=', 'news_tags.news_id') // Left join with tags table
                ->select('news.*', 'users.firstName', 'users.lastName', 'users.role', 'tags.tag') // Select the tag column from tags table
                ->orderBy('news.created_at', 'desc')
                ->get();
                dd($news);
