...
class AuthController extends Controller
{
    ...
    public function registerHero(HeroAuthRequest $request)
    {
        ...
        $heroRequest = HerosRequest::where('auth_id' ,$user['id'])->get();
        for($i=1;$i<=4;$i++)
        {
            $image = $request->file("image$i");
            $imageName =  $image->getClientOriginalName() . uniqid();
            $noSpacesString = str_replace(' ', '', $imageName);
            $image->move(public_path('assets/images/HerosImages'), $noSpacesString);
            H::create([
                'heros_requests_id' => $user->id,
                'image' => $path,
            ]);
            $heroRequest->image = $noSpacesString;
            $heroRequest->heros_requests_id = $heroRequest->id;
            $heroRequest->save();
        }
